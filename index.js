...
app.post("/doctorlogin", async (req, res) => {
    ...
    res.json({ status: "2FA_REQUIRED", message: "Verification code sent to email" });
...
app.post("/verify2fa", (req, res) => {
    ...
    const storedToken = temp2FATokens[email];
    
    if (!storedToken) {
    return res.json("No token found or token expired");
    }
    
    if (Date.now() > storedToken.expiresAt) {
    delete temp2FATokens[email];
